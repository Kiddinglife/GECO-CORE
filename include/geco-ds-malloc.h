...
class malloc_alloc
{
    ...
    static void* oom_malloc(size_t size)
    {
        ...
        while (result == 0)
        {
            ...
            if (my_oom_handler == 0) { THROW_BAD_ALLOC; }
            (*my_oom_handler)();
            result = malloc(size);
        }
        return result;
...
typedef typename malloc_alloc<0> malloc_alloc_0;

//! out-of-memory handling when malloc() fails
#ifndef GECO_STATIC_TEMPLATE_MEMBER_BUG
template <int inst>
...
{
    private:
    enum { extra_size = 8 };  //! Size of space used to store size.  Note
...
# if defined(__SUNPRO_CC) || defined(__GNUC__) || defined(__HP_aCC)
    static Chunk* GECO_VOLATILE free_list[];
    // Specifying a size results in duplicate def for 4.1
# else
    // 这里分配的free_list为16  
    // 对应的内存链容量分别为8, 16, 32 ... 128  
    static Chunk* GECO_VOLATILE free_list[NFREELISTS];
# endif
...
template <bool threads, int inst>
typename default_alloc<threads, inst>::Chunk* GECO_VOLATILE
